...
export class MessageSender {
     ...
     */
    constructor(registry, pushConnectionFactory = null) {
    ...
    sendInvocationsToServer() {
        ...
        if (hasActiveRequest || (this.#push !== null && !this.#push.isActive())) {
            ...
            Console.debug(`Postpone sending invocations to server because of ${hasActiveRequest ? 'active request' : 'PUSH not active'}`);
        }
        else {
            this.#doSendInvocationsToServer();
        }
     ...
     */
    sendRequest(reqInvocations, extraJson) {
    ...
    #preparePayload(reqInvocations, extraJson) {
        const payload = {};
     ...
     */
    setPushEnabled(enabled, reEnableIfNeeded = true) {
     ...
     */
    setClientToServerMessageId(nextExpectedId, force) {
        if (nextExpectedId === this.#clientToServerMessageId) {
            ...
            if (this.#pushPendingMessage !== null &&
                this.#pushPendingMessage[CLIENT_TO_SERVER_ID] < nextExpectedId) {
                this.#pushPendingMessage = null;
            }
            if (this.hasQueuedMessages()) {
                // If the queued message is the expected one, remove it and send next.
                if (this.#messageQueue[0][CLIENT_TO_SERVER_ID] + 1 === nextExpectedId) {
        ...
        if (nextExpectedId > this.#clientToServerMessageId) {
            if (this.#clientToServerMessageId === 0) {
                ...
                Console.warn(`Server expects next client-to-server id to be ${nextExpectedId} but we were going to use ${this.#clientToServerMessageId}. Will use ${nextExpectedId}.`);
            }
            this.#clientToServerMessageId = nextExpectedId;
        }
 ...
 */
export function sendBeacon(url, payload) {
